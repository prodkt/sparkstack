      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {selectedTheme ? (
              <>
                <Circle size={24} stroke={`var(--${selectedTheme.value || "lime"}-9)`} fill={`var(--${selectedTheme.value || "lime"}-a7)`} className={`focus-visible:rounded-full before:focus-visible:rounded-full after:focus-visible:rounded-full shrink-0 scale-[2] ring-offset-0 ring-offset-${selectedTheme.value || "lime"}-9 text-shadow-${selectedTheme.value || "lime"}-9 text-${selectedTheme.value || "lime"}-9 stroke-${selectedTheme.value || "lime"}-9 fill-${selectedTheme.value || "lime"}-a7`} />
        ...
        <PopoverContent className="p-0" side="right" align="start">
          <Command>
            ...
            <CommandList className="">
              ...
              <CommandGroup className={`w-full [&>_div]:grid [&>_div]:grid-cols-2 [&>_div]:grid-flow-row [&>_div]:gap-1 [&>_div]:w-full`}>
                {themes.map((theme) => (
                  <CommandItem
                  key={theme.value}
                  value={theme.value}
                  onSelect={(value) => {
                    const selectedTheme = themes.find((theme) => theme.value === value) || null
                    ...
                    className={`bg-inherit relative after:absolute after:inset-0 after:z-10 after:bg-[var(--${theme.value}-4)] after:opacity-0 after:transition-opacity after:duration-300 after:ease-in-out hover:after:opacity-100 basis-1/2 max-w-1/2 w-auto shrink grow-0 flex flex-col items-start justify-start gap-1 px-3 py-2 `}
                  >
                    ...
                    <Circle
                      ...
                      className={cn(
                        `h-4 w-4 shrink-0 !text-${theme.value || "lime"}-9 !stroke-${theme.value || "lime"}-9 !fill-${theme.value || "lime"}-9`,
